...
/// What the capture chain must deliver. Opus is a 48 kHz codec, so unlike the
/// PCM WebSocket transport there is no resampling to do.
pub const SESSION_SAMPLE_RATE: u32 = OPUS_SAMPLE_RATE;

/// The Codex desktop app's own realtime model, taken from its binary. This
/// transport reaches it; the public `/v1/realtime` WebSocket does not.
pub fn default_realtime_model() -> &'static str {
    MODEL
}

/// Session models for the settings dropdown. Only the app's own is known to
/// work over this transport — the `gpt-realtime-*` family lives on the public
/// endpoint and is offered by the WebSocket build instead.
pub const REALTIME_MODELS: &[(&str, &str)] = &[(MODEL, "Codex 앱과 동일")];

/// This transport has no separate transcription model: the session model
/// produces the user's transcript itself, over the events data channel. The
/// settings dropdown is therefore empty here.
pub fn default_transcription_model() -> &'static str {
    ""
}
pub const TRANSCRIPTION_MODELS: &[(&str, &str)] = &[];

/// Turn boundaries come from the server's VAD over the media track, exactly as
/// they do for the Codex app.
pub fn manual_turns() -> bool {
    false
}

/// Never: the transcript already comes from the session, and asking a
/// generative model to transcribe was measured inventing speech.
...
pub async fn transcribe_committed_audio(
    ...
    Err(RpcError::Spawn(
        "model transcription is not available on the WebRTC transport".into(),
    ))
}

/// Session model to connect with: settings, else the app's own.
...
impl RealtimeSession {
    ...
    /// Send digital silence without ending the turn — the lead-in padding that
    /// stops the model's windowing from eating the first syllable.
