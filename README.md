...
2. Hold Ctrl+E and speak — the pill reacts instantly
3. Release — the transcript is typed at the cursor

1. 타이핑할 곳 클릭 (터미널, 에디터 어디든)
2. Ctrl+E를 누른 채로 말하기 — pill이 즉시 반응
3. 손 떼기 → 커서 위치에 타이핑
```

Any key or chord works — F9, backquote, or a single key like CapsLock, whose
lock state the app puts back the way it found it. It's one field in settings.
Text is never injected while a modifier is still held, so chord hotkeys don't
leak Ctrl+letter shortcuts into the target app.
F9, 백틱, CapsLock 같은 단일 키 등 원하는 키/조합으로 설정에서 바꿀 수 있습니다
(CapsLock을 쓰면 캡스 상태는 앱이 원복합니다). 조합 키를 써도 수정자 키가 완전히
떨어진 뒤에 입력하므로 Ctrl+문자 단축키가 잘못 눌리지 않습니다.
