...
**Codex CLI 로그인 그대로 돌아가는 윈도우 음성 받아쓰기. API 키도, 받아쓰기 서비스
구독도, 새로 만들 계정도 없다.**

이 PC에서 `codex login`이 이미 되어 있다면 더 설정할 게 없다. Codex Mic는 CLI가
`~/.codex/auth.json`에 저장해둔 OAuth 토큰을 읽어 쓰고, 만료되면 알아서 갱신하고,
마이크를 Codex 데스크톱 앱이 쓰는 실시간 엔드포인트로 흘려보낸다. 앱이 따로
저장하는 자격 증명은 없다.
...
`codex login`은 OAuth 액세스 토큰, 리프레시 토큰, 계정 ID를 `~/.codex/auth.json`
(또는 `$CODEX_HOME/auth.json`)에 쓴다. Codex Mic는 그 파일을 읽어서 액세스 토큰을
베어러 토큰으로 보낸다. 인증은 이게 전부다.

액세스 토큰은 며칠이면 만료되는 JWT다. 만료 2분 전이 되면 앱이
`auth.openai.com/oauth/token`에 리프레시 토큰을 교환하고(CLI가 하는 것과 같은
요청) 결과를 `auth.json`에 다시 쓴다. 받아쓰기 하려고 다시 로그인할 일은 없다.

로그인이 없으면 시작할 때 pill이 알려준다. 네트워크를 찔러보지 않고 파일만 본다.
...
서명하지 않은 바이너리라 SmartScreen이 막는다. `추가 정보` → `실행`. 찜찜하면 직접
빌드해도 된다. 명령 하나다.
...
```powershell
cd src-tauri; cargo run --example hotkeys    # 다른 키가 되는지 확인
```
